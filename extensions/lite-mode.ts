 ...
 * 而是主会话直接编排单一 general agent，按阶段以 model= 传档位模型：
 *   small（检索/文档/找证据）= models.searcher
 *   medium（实现/常规计划）  = models.implementer
 *   large（咨询/修订计划/独立审查）= models.consultant
 * 档位从 config.models 实时投影——/sub-models 与预设改了自动同步，无独立配置表。
 *
...
/** 档位 → 模型投影：small=searcher / medium=implementer / large=consultant。 */
...
const CHAIN = (t: { small: string; medium: string; large: string }) =>
...
export function litePromptLines(cfg: LiteConfigLike): string[] {
	...
	if (mode === "on") {
		return [
			`Lite workflow mode: ON — 工作流请求在本会话内直接编排：不开 launch-tabs、不用 searcher/planner/implementer 等角色 agent，改用单一 general agent + 按阶段档位模型走 lite 链。`,
			CHAIN(tiers),
			DISCIPLINE,
			BOUNDARY,
		];
